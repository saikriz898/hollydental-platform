...
export default function LoginOverlay({ message = "Signing you in..." }: LoginOverlayProps = {}) {
  return (
    <div className="fixed inset-0 bg-navy z-50 flex items-center justify-center animate-fade-in" id="login-overlay">
      {/* Radial overlay */}
      ...
      <div className="relative z-10 text-center space-y-6 max-w-sm px-6">
        <div className="relative flex justify-center">
          <div className="w-16 h-16 rounded-full bg-gold/10 flex items-center justify-center border border-gold/20 animate-pulse">
            <Sparkles className="w-8 h-8 text-gold animate-spin" style={{ animationDuration: "5s" }} />
          </div>
        ...
        <div className="w-24 h-0.5 bg-white/10 mx-auto rounded-full overflow-hidden relative">
          <div 
            className="absolute top-0 h-full bg-gold rounded-full w-1/2 animate-shimmer"
            style={{ 
